"""
RUNS FULL STARDIST 3D ANALYSIS (WITH LUMEN ANALYSIS)
...
directory = r'\\istore.lmb.internal/mshahbazi_lab_share/Aly/3D_Spheroids/' + by + '/3D_Culture/'

output_directory = 'D:\\User Data\\Aly\\StarDist\\' + by + '\\' + culture + '\\' + experiment + '\\' + sub_experiment_0 + '\\'
...
path = directory + culture + '/' + experiment + '/' + sub_experiment_0
files = glob.glob(path + '/*.tif')
    
spheroid_path = path + '/' + 'Spheroids/' + 'Object Predictions'
spheroid_files = glob.glob(spheroid_path + '/*.h5')
    
lumen_path = path + '/' + 'Lumens/' + 'Object Predictions'
...
for file_idx in tqdm.tqdm(list(chain(file_indices[0:]))):
    ...
    sub_experiment = sub_experiment_0 + '\\'
        ...
        for j, spheroid_label in enumerate(u_spheroid[0:]):
            ...
            for idx, label in enumerate(np.unique(labels)[1:]):
                ...
                if np.unique(nuc_classifier[np.where(labels == label)])[0] == 1:  
                    nucleus_info["channel 2 expression"] = 'negative'
                    
                elif np.unique(nuc_classifier[np.where(labels == label)])[0] == 2:  
                    nucleus_info["channel 2 expression"] = 'positive'
                    positive_count += 1
                    
                else:
                    nucleus_info["channel 2 expression"] = 'error'
                
                nucleus_info["nearest lumen"] = nearest_lumen
                nucleus_info["nearest lumen distance"] = float(str(np.round(np.min(lum_nuc_dist),3)))
                nucleus_info["normalised nearest lumen distance"] = float(str(np.round(abs(np.linalg.norm(nearest_lum_nuc_vec / sph_dim)),3))) # rotated lumen-nucleus vectors normalised to rotated spheroid dimensions
                nucleus_info["count-normalised nearest lumen distance"] = float(str(np.round(abs(np.linalg.norm((nearest_lum_nuc_vec / (sph_dim*packing_correction[num_lumen])))),3))) # rotated lumen-nucleus vectors normalised to rotated spheroid dimensions
